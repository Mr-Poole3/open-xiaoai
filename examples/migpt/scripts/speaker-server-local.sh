...
ssh "${SPEAKER_SSH_OPTS[@]}" "$SPEAKER_SSH" "mkdir -p /data/open-xiaoai && echo '${LOCAL_SERVER}' > /data/open-xiaoai/server.txt && pkill -f '/data/open-xiaoai/client' || true; sleep 1; /data/open-xiaoai/client \"\$(cat /data/open-xiaoai/server.txt)\" >/dev/null 2>&1 &"
