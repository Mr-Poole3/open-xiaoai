/**
 * Render config.ts on VPS from environment variables (GitHub Secrets at deploy).
 * Usage: DOUBAO_APP_ID=... DOUBAO_ACCESS_KEY=... npx tsx scripts/render-deploy-config.ts > config.ts
 */
import { writeFileSync } from "node:fs";
import { resolve } from "node:path";

const required = ["DOUBAO_APP_ID", "DOUBAO_ACCESS_KEY"] as const;
...
export const kOpenXiaoAIConfig: OpenXiaoAIConfig = {
  ...
  callAIKeywords: ${JSON.stringify(
    (process.env.MIGPT_CALL_AI_KEYWORDS ?? "请,你").split(",").map((s) => s.trim())
  )},
};
`;
